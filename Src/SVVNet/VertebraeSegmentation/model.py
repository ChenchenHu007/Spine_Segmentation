...
class Encoder(nn.Module):
    def __init__(self, in_ch, list_ch):
        ...
        self.encoder_4 = nn.Sequential(
            SingleConv(list_ch[3], list_ch[4], kernel_size=3, stride=(2, 2, 2), padding=1),
            SingleConv(list_ch[4], list_ch[4], kernel_size=3, stride=1, padding=1)
        )
        self.encoder_5 = nn.Sequential(
            SingleConv(list_ch[4], list_ch[5], kernel_size=3, stride=(2, 2, 2), padding=1),
...
class Decoder(nn.Module):
    def __init__(self, list_ch):
        ...
        self.upconv_4 = UpConv(list_ch[5], list_ch[4],scale_factor=(2, 2, 2))
        self.decoder_conv_4 = nn.Sequential(
            SingleConv(2 * list_ch[4], list_ch[4], kernel_size=3, stride=1, padding=1),
            SingleConv(list_ch[4], list_ch[4], kernel_size=3, stride=1, padding=1)
        )
        self.upconv_3 = UpConv(list_ch[4], list_ch[3],scale_factor=(2, 2, 2))
